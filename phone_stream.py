...
function refresh(){fetch('/status').then(r=>r.json()).then(s=>{const u=s.remote||s.lan||'',fps=typeof s.stream_fps==='number'?' &middot; '+s.stream_fps.toFixed(1)+' FPS':'',aud=s.audio_level===undefined?'':' &middot; '+(s.audio_level>0.003?'\u58f0\u97f3\u6b63\u5e38':'\u7b49\u5f85\u58f0\u97f3');document.getElementById('urlbar').innerHTML=(s.remote?'\u516c\u7f51':'\u5c40\u57df\u7f51')+'\u5730\u5740: <a href="'+u+'">'+u+'</a>'+fps+aud;const b=document.getElementById('modeBtn'),on=s.detection_enabled!==false;b.textContent=on?'\u5207\u5230\u76d1\u63a7':'\u5f00\u542f\u8bc6\u522b';b.className='btn'+(on?'':' on');const cb=document.getElementById('camBtn');if(cb){cb.textContent=s.multi_camera?'\u5207\u56de\u5355\u6444':'\u591a\u6444\u540c\u5c4f';cb.className='btn'+(s.multi_camera?' on':'')}}).catch(()=>{})}setInterval(refresh,3000);refresh();loadAudioDevices();
function toggleMode(){fetch('/cmd?action=detection_toggle').then(()=>setTimeout(refresh,120))}function toggleMulti(){const b=document.getElementById('camBtn');b.textContent='\u5207\u6362\u4e2d...';fetch('/cmd?action=multi_toggle').then(()=>setTimeout(refresh,2200))}function snap(){fetch('/cmd?action=snapshot').then(()=>{const b=document.getElementById('snapBtn');b.textContent='\u5df2\u62cd\u7167\u2713';setTimeout(()=>b.textContent='\u62cd\u7167',900)})}function rec(){fetch('/cmd?action=record').then(r=>r.text()).then(t=>{const b=document.getElementById('recBtn'),on=t.includes('on');b.textContent=on?'\u505c\u6b62\u5f55\u5236':'\u5f55\u5236';b.className='btn'+(on?' rec':'')})}
