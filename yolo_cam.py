...
def save_snapshot(frame, label, conf, full_frame=False):
    ...
    try:
        cv2.imwrite(str(fpath), watermarked, [cv2.IMWRITE_JPEG_QUALITY, 90])
...
def build_buttons(w, h):
    ...
    if sel:
        ...
        cam_y = y - btn_h - gap
        btn_mgr.add(Button(
            start_x, cam_y, cam_w, btn_h,
            f"◀ 摄像头{cur}", lambda: do_switch_cam(-1),
            color=(40, 80, 140), text_color=(255, 255, 255), font_size=12
        ))
        btn_mgr.add(Button(
            start_x + cam_w + gap, cam_y, cam_w, btn_h,
            f"摄像头{cur} ▶", lambda: do_switch_cam(1),
            color=(40, 80, 140), text_color=(255, 255, 255), font_size=12
        ))
...
while running:
    ...
    tmp_font = ImageFont.truetype(font_path, 16) if font_path else ImageFont.load_default()
    bbox = tmp_d.textbbox((0, 0), date_str, font=tmp_font)
    text_w = bbox[2] - bbox[0] + 8
    date_x = w - text_w - 10
    ann = draw_cn(ann, date_str, (date_x, 8), 16, (255, 255, 200), (0, 0, 0))
